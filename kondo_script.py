    ...
    def detect (self, img)
        ...
        for blob in detected_blobs:
            ...
            if blob.roundness() > 0.9:
                ...
                b=image.rgb_to_lab(img.get_pixel(blob.x()+blob.w()-1, blob.y()))
                c=image.rgb_to_lab(img.get_pixel((blob.x()+blob.w()-1), (blob.y()+blob.h()-1)))
                d=image.rgb_to_lab(img.get_pixel(blob.x(), blob.y()+blob.h()-1))
...
while(True):
    ...
    action = strategy.generate_action (local)
